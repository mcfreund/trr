...
s_summarystat_pop <- s_summarystat_subj %>%
  ...
  mutate(hilo = hi - lo) %>%
  group_by(subj, roi, session, response) %>%  ## average over wave
  summarize(hilo = mean(hilo)) %>%
  group_by(roi, session, response) %>% 
  summarize(
    Estimate = mean(hilo),
...
model_info_fixef$hyp <- "(hilo_wave1 + hilo_wave1)/2 = 0"
...
s_hbm_posterior <- s_hbm_posterior %>% mutate(network = get_network(roi_nm, TRUE))
...
p_brain_eg <-
  ggplot() +
  geom_brain(mapping = aes(fill = region %in% example_rois), atlas = atlas, hemi = "left", side = "lateral") +
  theme_surface +
  theme_void() +
  theme(legend.position = "none") +
...
p_trrpost_eg <-
  ...
  # geom_vline(
  #   data = s_hbm_posterior[
  #     roi_nm %in% example_rois & session == "baseline",
  #     .(trr_05 = quantile(trr, 0.05)), by = c("response", "roi_nm")
  #     ],
  #   aes(xintercept = trr_05, color = response)
  # ) +
