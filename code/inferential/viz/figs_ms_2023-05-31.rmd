...
path_figs <- here("figs", "ms_2023-05-31")
```
...
To define the z-score, I scaled the mean population hi-lo effect by its standard deviation across subjects (estimated as a subject-level variance component).
  ...
  - The first method I tried didn't work well. I first tried scaling each posterior sample of the mean by the SD, per sample of the posterior. However a fair number of samples in some regions had very small SDs, so the z-score blew up when doing it this way. Instead I summarized over posterior samples, then calculated the z-score.
...
While the units of z-score may be more intuitive than t-statistic, I think the t-statistic makes more conceptual sense in our case. It shows the the effect size relative to the overall uncertainty in the effect size---which (I am pretty sure) integrates the influence of both trial and subject-level variability.
...
p_trr_stats_uv <- 
  ...
  guides(fill = guide_colorbar(title.position = "left", title.vjust = 0.9)) +
  geom_text(
    data = data.frame(x = -50, y = 100, label = c("ICC", "HBM"), model_nm = c("summarystat", "no_lscov_symm")),
...
p_delta_trr_line <- trr_stats_uv %>%
  mutate(
    is_roi = region %in% s_pop_t_brain[is_roi == TRUE, region],
    model_nm = ifelse(model_nm == "summarystat", "ICC", "HBM"),
    model_nm = factor(model_nm, levels = c("ICC", "HBM"))
    ) %>%
  group_by(is_roi, model_nm) %>%
  ...
  geom_point(stat = "summary", fun = "mean", size = 4, color = "white", shape = 21) +
  ...
  labs(
    x = "Mean hi-lo contrast (t)",
    y = "TRR"
  )

p_delta_trr_scatter <- trr_stats_uv %>%  
...
p_trr_uv <-
  (p_trr_stats_uv / (p_delta_trr_line | p_delta_trr_scatter)) +
  plot_layout(heights = c(1.5, 1)) + plot_annotation(tag_levels = list(c("A", "B")))
ggsave(
  file.path(path_figs, "trr_diff_uv.pdf"),
  dev = cairo_pdf, width = 6, height = 4.5
  )
p_trr_uv
...
p_trr_icc_uvmv_scatter <- 
  r_summarystat %>%  
  filter(session == "baseline") %>%
  mutate(is_roi = region %in% s_pop_t_brain[is_roi == TRUE, region]) %>%
...
p_trr_icc_uvmv <-
  (p_trr_icc_uvmv_brain / (p_trr_icc_uvmv_line | p_trr_icc_uvmv_scatter)) +
  plot_layout(heights = c(1.5, 1)) + plot_annotation(tag_levels = list(c("A", "B")))
ggsave(
...
p_trr_sd_uvmv_scatter <-
  s_hbm_posterior_sum %>%
  filter(session == "baseline", variable == "trr_sd") %>%
  mutate(is_roi = region %in% s_pop_t_brain[is_roi == TRUE, region], value = 1/value) %>%
...
p_trr_sd_uvmv <-
  (p_trr_sd_uvmv_brain / (p_trr_sd_uvmv_line | p_trr_sd_uvmv_scatter)) +
  plot_layout(heights = c(1.5, 1)) + plot_annotation(tag_levels = list(c("A", "B")))
ggsave(
...
d_tab <- full_join(
  ...
  full_join(s_pop_t_brain %>% rename(t_stat = value), by = "region") %>%
...
p_eg_scatter <- d_tab %>%
  ...
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0)  +
  geom_point(data = . %>% filter(is_roi == FALSE), shape = 21, size = 2) +
  geom_point(data = . %>% filter(is_roi == TRUE), shape = 21, size = 2) +
  scale_color_manual(values = c(`FALSE` = "black", `TRUE` = "firebrick")) +
  scale_fill_manual(values = c(`FALSE` = "transparent", `TRUE` = "firebrick")) +
  labs(
    x = "\u0394Precision(TRR), multiv.\u2212univ.\n(log ratio)",
    y = "\u0394TRR (univar.), HBM\u2212ICC\n(atanh(r))"
    ) +
  ...
  geom_text(
    data = . %>%
      ...
      summarise(n = n()) %>%
      mutate(
        lr = ifelse(trrsd_response == "multiv. > univar.", 1, -0.5),
        trr_diff = ifelse(trr_model == "HBM > ICC", 0.25, -0.25),
        is_roi = FALSE
      ),
    aes(label = n), size = 10
...
th <- list(
  tt = list(
    ...
    geom_text(
      data = data.frame(
        region = "LH_DorsAttnA_SPL_7", trr = -Inf, y = Inf, label = c("univar.", "\nmultiv."),
        response = c("uv", "rda")
        ),
      aes(label = label, y = y), vjust = 1, hjust = 0
    )
  ),
  ft = theme(
    axis.line.y.left = element_blank(),
...
for (i in seq_along(eg)) {
  p <- s_hbm_posterior[roi_nm %in% eg[[i]] & session == "baseline"] %>%
    ...
    facet_wrap(
      vars(gsub("17Networks_", "", region)), ncol = 2,
      scales = "free_y"
      ) +
...
ggsave(
  ...
  p_eg,
  dev = cairo_pdf, width = 9, height = 4.5
  )
...
p_ratio <-
  ...
  mutate(value = log(value), is_roi = region %in% top_t) %>%
  ...
  ggplot(aes(precision_mvuv, variratio_mvuv, color = is_roi, fill = is_roi, group = "")) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  geom_vline(xintercept = 0, linetype = "dotted")  +
  stat_ellipse(type = "t", linetype = "dotted") +
  geom_abline(slope = -1, linetype = "dotted")  +
  geom_point(data = . %>% filter(!is_roi), shape = 21, size = 2) +
  geom_point(data = . %>% filter(is_roi), shape = 21, size = 2) +
  scale_color_manual(values = c(`FALSE` = "black", `TRUE` = "firebrick")) +
  scale_fill_manual(values = c(`FALSE` = "transparent", `TRUE` = "firebrick")) +
  labs(
    x = "\u0394Precision(TRR), multiv.\u2212univ.\n(log ratio)",
    y = "\u0394Variab. Ratio, multiv.\u2212univ.\n(log ratio)"
    ) +
  theme(legend.position = "none") +
  geom_text(
    data = . %>%
...
ggsave(
  ...
  p_ratio,
  dev = cairo_pdf, width = 4.5, height = 4
  )
