...
s_hbm_posterior[, .(trr = max_aposteriori(trr)), by = c("model_nm", "roi_nm", "response")] %>%
  ...
  #pivot_wider(id_cols = c("roi_nm"), names_from = response, values_from = trr_bias) %>%
  ggplot(aes_string("summarystat", model, color = "roi_nm", fill = "roi_nm")) +
  geom_abline() +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  geom_point(shape = 21, size = point_size) +
...
s_hbm_posterior[, .(trr = max_aposteriori(trr), trr_sd = sd(trr)), by = c("model_nm", "roi_nm", "response")] %>%
  full_join(r_summarystat %>% rename(summarystat = r) %>% ungroup %>% select(-model_nm)) %>%
  mutate(trr_bias = trr - summarystat) %>%
  ggplot(aes(trr_sd, summarystat, color = roi_nm, fill = roi_nm)) +
  geom_point(shape = 21, size = point_size) +
  geom_hline(yintercept = 0) +
  scale_color_viridis_d() +
  scale_fill_viridis_d() +
...
s_hbm_posterior[, .(trr = max_aposteriori(trr), trr_sd = sd(trr)), by = c("model_nm", "roi_nm", "response")] %>%
  full_join(r_summarystat %>% rename(summarystat = r) %>% ungroup %>% select(-model_nm)) %>%
  mutate(trr_bias = trr - summarystat) %>%
  ggplot(aes(trr_sd, trr_bias, color = roi_nm, fill = roi_nm)) +
  geom_point(shape = 21, size = point_size) +
  geom_hline(yintercept = 0) +
  scale_color_viridis_d() +
  scale_fill_viridis_d() +
  theme(legend.position = "none") +
  facet_grid(cols = vars(response)) +
  labs(title = "TRR bias ~ SD(HBM TRR)")
...
s_hbm_posterior[, .(trr = max_aposteriori(trr), trr_sd = sd(trr)), by = c("model_nm", "roi_nm", "response")] %>%
  full_join(r_summarystat %>% rename(summarystat = r) %>% ungroup %>% select(-model_nm)) %>%
  mutate(trr_bias = trr - summarystat) %>%
  full_join(s_hbm_subj_ratio[, .(ratio = max_aposteriori(ratio)), by = c("model_nm", "roi_nm", "response")]) %>%
  #pivot_wider(names_from = response, values_from = ratio) %>%
  ggplot(aes(trr_bias, ratio, color = roi_nm, fill = roi_nm)) +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  geom_point(shape = 21, size = point_size) +
  scale_color_viridis_d() +
