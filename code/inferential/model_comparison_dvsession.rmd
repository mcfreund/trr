...
s_hbm_subj_ratio %>%
  ggplot(aes(log(ratio), color = model_nm)) +
  geom_density(size = 1) +
  facet_wrap(vars(roi_nm), labeller = labeller(roi_nm = function(x) gsub("17Networks_", "", x))) +
  scale_color_manual(values = colors_model[models_hbm]) +
  theme(legend.position = c(0.75, 0.05)) +
...
s_hbm_subj_ratio[, .(ratio = max_aposteriori(ratio)), by = c("model_nm", "roi_nm")] %>%
  ...
  theme(legend.position = "none") +
  labs(title = "MAP(sd(trial)/sd(subj_stroop))", caption = "each line is an ROI")

s_hbm_subj_ratio[, .(ratio = mean(ratio)), by = c("model_nm", "roi_nm")] %>%
  ggplot(aes(model_nm, ratio, color = roi_nm)) +
  geom_boxplot(fill = "grey50", color = "black", width = 1/3) +
  geom_line(aes(group = roi_nm), alpha = 0.5) +
  scale_color_viridis_d() +
  scale_y_continuous(limits = c(0, 100)) +
  theme(legend.position = "none") +
  labs(title = "mean(sd(trial)/sd(subj_stroop))", caption = "each line is an ROI")

s_hbm_subj_ratio[, .(ratio = median(ratio)), by = c("model_nm", "roi_nm")] %>%
  ggplot(aes(model_nm, ratio, color = roi_nm)) +
  geom_boxplot(fill = "grey50", color = "black", width = 1/3) +
  geom_line(aes(group = roi_nm), alpha = 0.5) +
  scale_color_viridis_d() +
  scale_y_continuous(limits = c(0, 100)) +
  theme(legend.position = "none") +
  labs(title = "median(sd(trial)/sd(subj_stroop))", caption = "each line is an ROI")

s_hbm_subj_ratio[, .(ratio = sd(ratio)), by = c("model_nm", "roi_nm")] %>%
  ggplot(aes(model_nm, ratio, color = roi_nm)) +
  geom_boxplot(fill = "grey50", color = "black", width = 1/3) +
  geom_line(aes(group = roi_nm), alpha = 0.5) +
  scale_color_viridis_d() +
  scale_y_continuous(limits = c(0, 100)) +
  theme(legend.position = "none") +
  labs(title = "sd(sd(trial)/sd(subj_stroop))", caption = "each line is an ROI")
