...
file_name <- switch(
  ...
  paste0("projections__stroop__rda_lambda_", shrinkage_factor, "__n_resamples", n_resamples, "__cv_allsess.csv"), ## F
  paste0("projections__stroop__", classifier, "__n_resamples", n_resamples, "__cv_allsess.csv")  ## TRUE
)
...
allres <-
  ...
  foreach(wave_i = seq_along(waves), .inorder = FALSE, .combine = "rbind") %dopar% {
    ...
    result <- rbindlist(projs_all, idcol = "test_session")
    return(result)
