...
file_name <- paste0(
  ...
  "__cv_runs_wave", do_waves[1], do_waves[2], ".csv"
)
...
allres <-
  ...
  foreach(wave_i = seq_along(waves), .inorder = FALSE, .combine = "c") %dopar% {
    ...
    tryCatch(
      ...
      stopifnot(length(trials) == 1)  ## for baselinse
      trials <- trials[sort(names(trials))]  ## sorts baseline, proactive, reactive
      names(trials) <- "baseline"#c("baseline", "proactive", "reactive")
      ...
      for (session_i in seq_along(trials)) {
        ...
        colnames(trials_session_c) <- paste0(y, "__", run_labels)
      ...
      for (session in sessions) {
        ...
        data_clean_rois_session <- data_clean_rois[[session]]

        projs_all_sess <- mfutils::enlist(c("run1", "run2"))
        weights_all_sess <- mfutils::enlist(c("run1", "run2"))
        for (test in 1:2) {  ## runs

          train <- setdiff(1:2, test)
          idx_shift <- n_trialspr[paste0(task_val, "_", session)]*(train - 1)
          resamples_train <- resamples[[paste0(subj_val, "__", wave_val, "__", session, "__", train)]]
          ...
          for (roi_i in seq_along(rois)) {
            ...
            data_clean_roi <- data_clean_rois_session[[rois[roi_i]]]  ## vertex by trial*run
            data_clean_roi <- list(
                run1 = data_clean_roi[, grep("run1", colnames(data_clean_roi))],
                run2 = data_clean_roi[, grep("run2", colnames(data_clean_roi))]
              )  ## list of vertex by trial matrices, of length n_run
            ## rename columns:
            for (run_i in seq_along(data_clean_roi)) {
              colnames(data_clean_roi[[run_i]]) <- gsub("__run.$", "", colnames(data_clean_roi[[run_i]]))
            }
            ## remove vertices with no BOLD variance
            is_good_vertex_run <- lapply(data_clean_roi, function(x) !is_equal(Var(x, 1, na.rm = TRUE), 0))
            is_good_vertex <- Reduce("&", is_good_vertex_run)  ## intersection
            ...
            d_test <- t(do.call(cbind, data_clean_roi_goodverts[paste0("run", test)]))
            colnames(d_test) <- paste0("v", seq_len(ncol(d_test)))
            l_train <- data_clean_roi_goodverts[paste0("run", train)]
            ...
            fits <- lapply(
              seq_len(n_resamples),
              function(i) {
                idx <- resamples_train[i, ] - idx_shift
                X <- l_train[[1]][idx, ]
                y <- rownames(X)

                if (any(is.na(X))) {
                  stop(paste("NA found in subject", subj_val,
                    wave_val, "session", session_val, "run", i))
                }
          ...
          weights_all_sess[[test]] <- res_weights

        }

        projs_all[[session]] <- rbindlist(projs_all_sess, idcol = "test_run")
        weights_all[[session]] <- rbindlist(weights_all_sess, idcol = "test_run")

      }

      result <- rbindlist(projs_all, idcol = "session", fill = TRUE)
      result_weights <- rbindlist(weights_all, idcol = "session", fill = TRUE)
...
res <-
  out[, .(value.rda = mean(value.rda, na.rm = TRUE), uv = mean(uv, na.rm = TRUE)), by = c("roi", "variable", "subj", "wave")] %>%
  dcast(... ~ variable, value.var = c("uv", "value.rda")) %>%
  mutate(uv = uv_hi - uv_lo, rda = value.rda_hi - value.rda_lo) %>%
  dcast(roi + subj ~ wave, value.var = c("uv", "rda")) %>%
  group_by(roi) %>%
  summarize(
    r_uv = cor(uv_wave1, uv_wave2),
    r_rda = cor(rda_wave1, rda_wave2)
  )
...
out[, .(value.rda = mean(value.rda, na.rm = TRUE), uv = mean(uv, na.rm = TRUE)), by = c("roi", "variable", "subj", "wave")] %>%
  dcast(... ~ variable, value.var = c("uv", "value.rda")) %>%
  mutate(uv = uv_hi - uv_lo, rda = value.rda_hi - value.rda_lo) %>%
  .[, .(uv = t.test(uv)$statistic, rda = t.test(rda)$statistic), by = c("roi", "wave")] %>%
  

length(unique(out$subj))
library(ggplot2)
res %>%
  ggplot(aes(r_uv, r_rda)) +
  geom_abline() +
  geom_point(aes(color = grepl("PFCl", roi)))

res %>% slice_max(r_rda, n = 20)
res %>% slice_max(r_uv, n = 20)

res %>%
  filter(grepl("PFCl", roi)) %>%
  View

res %>%
  filter(grepl("17Networks_LH_ContA_PFCl*", roi))

res %>%
  mutate(r_diff = atanh(r_uv - r_rda)) %>%
  ggplot(aes(1, r_diff)) +
  geom_boxplot()
...
out[, .(value.rda = mean(value.rda, na.rm = TRUE), uv = mean(uv, na.rm = TRUE)), by = c("roi", "variable", "subj", "wave")] %>%
  ...
  dcast(roi + subj ~ wave, value.var = c("uv", "rda")) %>%
  filter(grepl("17Networks_LH_ContA_PFCl*", roi)) %>%
  ggplot(aes(rda_wave1, rda_wave2)) +
  geom_point() +
  facet_wrap(vars(roi))


fname <- "/data/nil-external/ccp/chenr/trr/out/spatial/projections__stroop__rda__n_resamples100__demean_run__cv_allsess.csv"
orig <- fread(fname)
orig <- orig[test_session == "baseline"]
cor(
  orig[roi == "17Networks_LH_ContA_PFCl_2" & subj == "132017" & wave == "wave2", uv],
   out[roi == "17Networks_LH_ContA_PFCl_2" & subj == "132017" & wave == "wave2", value.rda]
)
...
orig %>%
  group_by(roi, subj, wave) %>%
  summarize(r_ridge = cor(uv, value.ridge), r_rda = cor(uv, value.rda)) %>%
  ggplot(aes(r_ridge, r_rda)) +
  geom_point() +
  geom_abline() +
...
out %>%
  ...
  scale_x_continuous(limits = c(-1, 1))
   ...
   cbind(
    orig %>% dplyr::select(roi, rda_orig = value.rda, subj, wave, trial),
    out  %>% dplyr::select(roi, value.rda, trial, subj, wave)
    ) %>%
  group_by(roi, subj, wave) %>%
  summarize(r_ridge = cor(uv, value.ridge), r_rda = cor(uv, value.rda)) %>%
  ggplot(aes(r_ridge, r_rda)) +
