...
behav_wave3 <- list(
  Axcpt  = fread(here("in", "behav", "orig", "dmcc4_behavior-and-events_axcpt_2021-09-23.csv")),
  Cuedts = fread(here("in", "behav", "orig", "dmcc4_behavior-and-events_cuedts_2021-09-23.csv")),
  Stern  = fread(here("in", "behav", "orig", "dmcc4_behavior-and-events_sternberg_2021-09-23.csv")),
  Stroop = fread(here("in", "behav", "orig", "dmcc4_behavior-and-events_stroop_2021-09-23.csv"))
)

behav <- pmap(list(behav_wave1, behav_wave2, behav_wave3),
  ~ (rbind(..1, ..2, ..3, idcol = "wave")))



## subset subjs and waves

behav <- lapply(behav, function(x) x[subj %in% subj_list & wave %in% do_waves])
...
map2(behav, here("in", "behav",
  paste0("behavior-and-events_wave", do_waves[1], do_waves[2], "_", tasks, ".csv")),
  fwrite)
fwrite(b, here("in", "behav",
