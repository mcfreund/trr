...
fs_new_mean <- summarise(fs_new, ridge = mean(ridge), uv = mean(uv))
fig <- ggplot(fs_new) +
  aes(x = uv, y = ridge) +
  geom_point(alpha = 0.5) +
  geom_point(data = fs_new_mean, col = "red") +
  geom_abline(intercept = 0, slope = 1) +
  geom_rug() +
  labs(x = "uv", y = "ridge",
...
fs_diff_mean <- summarise(fs_diff, ridge_diff = mean(ridge_diff), uv_diff = mean(uv_diff))
fig <- ggplot(fs_diff) +
