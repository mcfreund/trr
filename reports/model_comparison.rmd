...
res_diff_dat <- dat %>%
  filter(Term == "TRR") %>%
  get_diff_dat(name_term = "response", id_term = c("region", "model"))
```
...
We can see that generally, `r paste(tmp$model, collapse = " > ")`.
...
```{r, diff-elpd-loo, echo = FALSE, message = FALSE, warning = FALSE, fig.dim = c(16, 12)}

# Get data
diff_dat <- get_diff_dat(dat %>% filter(Term == "elpd_loo"), name_term = "model",
  id_term = c("region", "response"))

# Deal with `fixed_sigma`
tmp <- range(diff_dat %>%
  filter(model == paste("fixed_sigma", "-", .env$models[[1]])) %>%
  select(Estimate)
)
print(paste0("Range of the difference in elpd_loo between fixed_sigma and ", models[[1]],
  " models: (", tmp[[1]], ", ", tmp[[2]], ")"))

ylims <- range(diff_dat %>%
  filter(model != paste("fixed_sigma", "-", .env$models[[1]])) %>%
  select(Estimate)
)
...
Here we plot the distribution of the data in the previous figure over 32 ROIs:
...
This is a similar plot but for the difference between models:

```{r, TRR-mdl-diff-hist, echo = FALSE, message = FALSE, fig.dim = c(16, 9)}
ylims <- range(mdl_diff_dat$Estimate)
ylims <- ylims + c(-0.1, 0.1) * (ylims[[2]] - ylims[[1]])
for (i in seq_along(responses)) {
  res <- responses[[i]]
...
Here we plot the distribution of TRR over the core32 parcels: (pending)

```{r, TRR-hist, echo = FALSE, message = FALSE, fig.dim = c(16, 9), eval = FALSE}
ylims <- range(dat %>%
  filter(Term == "TRR") %>%
  select(Estimate)
)
...
Here we plot the TRR over core32 parcels and compare it with the old models we fit, which use either `mda::fda()`("ridge") or `sparsediscrim::lda_schafer()`("rda_full" and "rda_diag") to generate the response variable. The TRR model is similar to `"fix_sigma"` model but the fix effects are treatment-coded. Each figure is for one TRR model while each violin plot is for one response type.

(figure removed)

```{r, TRR-hist-core32, eval = FALSE, echo = FALSE, message = FALSE, fig.dim = c(21, 9)}
...
```{r, TRR-diff-hist-core32, eval = FALSE, echo = FALSE, message = FALSE, fig.dim = c(21, 9)}
