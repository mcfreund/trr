...
# Model 3: single session, full model, sans location-scale correlations
...
\begin{equation}
	\begin{split}
        ...
        & & & \Sigma_{55} & \dots & \Sigma_{58} & \\
        & & & \vdots & \ddots & \vdots & \\
        & & & \Sigma_{85} & \dots  & \Sigma_{88} &
      \end{bmatrix} \\
	\end{split}
\end{equation}
$$

# Model 4: single session, full model, sans location-scale correlations, with chen2021-style correlations


## notation

- conditions re-parameterized to contrast-coding scheme: $c \in \{\text{mean}, \text{stroop}\}$,
where $\text{mean}$ refers to the average score (incongruent + congruent), and stroop refers to the contrast (incongruent - congruent)/2. 
